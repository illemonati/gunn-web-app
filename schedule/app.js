...
export function scheduleApp (options = {}) {
    ...
    function getUsefulTimePhrase (minutes) {
        if (options.compact)
            return `${Math.floor(minutes / 60)}:${('0' + (minutes % 60)).slice(
                -2
            )}`
    ...
    function getSchedule (d, includeZero = options.show0) {
        ...
        if (periods.length) {
            if (
                options.hPeriods[weekday] &&
                !periods.find(pd => pd.name === 'H')
            ) {
            ...
            if (includeZero && !periods.find(pd => pd.name === '0')) {
                if (
                    getSchedule(new Date(ano, mez, dia - 1), false).periods
                        .length
                ) {
        ...
        if (periods.length) {
            ...
            if (checkfuture) {
                    ...
                    str = `<p class="schedule-endingin">${localizeWith(
                        ...
                        'times',
                        {
                            ...
                            T: `<strong>${(compactTime = getUsefulTimePhrase(
                                totalminute -
                                    periods[periods.length - 1].end.totalminutes
                            ))}</strong>`
                        }
                    )}</p>`
                ...
                // after school (endOfDay is an hour past)
                    ...
                    compactStr = localizeWith('ending-short', 'times', {
                        T: compactTime
                    })
                }
                // during a period
            ...
            for (const period of periods) {
                ...
                if (period.name === 'GT') {
                    innerHTML += `<span>${localize(
                        'gunn-together/subtitle'
                    )}</span>`
                }
                ...
                if (period.name === 'Lunch' && dayToPrime[weekday]) {
                    ...
                    if (clubs.length) {
                        innerHTML +=
                            `<span class="small-heading">${localize(
                                'lunch-clubs'
                            )}</span>` +
                            clubs
                                .map(
    ...
    function getNext (timeOk, { start = true, end = true } = {}) {
        ...
        for (const period of schedule.periods) {
            if (
                start &&
                timeOk(period.start.totalminutes * 60, time, period.name)
            ) {
                return {
                    period: period.name,
                    time:
                        period.start.totalminutes * 60 * 1000 + today.getTime(),
                    type: 'start'
                }
            }
            if (
                end &&
                timeOk(period.end.totalminutes * 60, time, period.name)
            ) {
                return {
                    period: period.name,
        ...
        if (options.openLinkBefore !== null) {
            const next = getNext(
                (pdTime, nowTime, pdName) =>
                    getPeriod(pdName).link &&
                    pdTime - options.openLinkBefore > nowTime,
                { end: false }
            )
            if (next) {
                return {
                    ...next,
                    time: next.time - options.openLinkBefore * 1000
                }
            }
        }
        return null
            ...
            function checkMinute () {
                ...
                if (nextNotif) {
                    if (currentTime() >= nextNotif.showTime) {
                        const today = getDate(now())
                        const currentMinute =
                            (currentTime() - today.getTime()) / 1000 / 60
                ...
                if (nextLinkOpen) {
                    if (currentTime() >= nextLinkOpen.time) {
                        if (options.openLinkInIframe) {
                            ...
                            window.open(
                                getPeriod(nextLinkOpen.period).link,
                                '_blank'
                            )
                        }
                        nextLinkOpen = getNextLinkOpen()
