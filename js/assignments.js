...
class Assignment {
    ...
    get algorithmicValue () {
        return (
            this.due - this.sortableImportance * IMPORTANCE_ALGORITHMIC_WEIGHT
        )
    }
    ...
    static dateObjToDayInt (dateObj) {
        const minutes =
            dateObj.getTime() / 1000 / 60 - dateObj.getTimezoneOffset()
    ...
    setStatus (status = 'idk', type = 'load') {
        ...
        switch (status) {
            ...
            case 'loaded':
                this.status.textContent =
                    type === 'load'
                        ? localize('assync-loaded')
                        : localize('assync-saved')
    ...
    save (mode, asgn) {
        ...
        return fetch(
            ...
            .then(({ ok }) =>
                ok
                    ? null
                    : Promise.reject(new Error('Assignment failed to save'))
            )
            .catch(err => {
                logError(err)
...
export function initAssignments ({
    ...
    if (failQueueCookie) {
        ...
        manager.saveFailures = () => {
            cookie.setItem(
                failQueueCookie,
                JSON.stringify(manager.failureQueue)
            )
        }
    }
