        ...
        json => {
            ...
            for (const name of names) {
                ...
                Object.assign(li.dataset, {
                    ...
                    search: [
                        name,
                        ...searchableProps.map(prop => item[prop] || '')
                    ].join(' ')
                })
                ripple(li)
...
function egg () {
    ...
    const scoreDisplay = wrapper
        ...
        .appendChild(document.createTextNode('[press play to start]'))
        .parentNode
    ...
    const render = () => {
        ...
        c.fillStyle = document.body.classList.contains('dark')
            ? 'white'
            : 'black'
        ...
        e => {
            ...
            if (stats.count < price) {
                priceDisplay.textContent =
                    price + ' (which is too many for you)'
            } else {
                stats.count -= price
                clicks.textContent = stats.count
                cookie.setItem('[gunn-web-app] scheduleapp.clicks', stats.count)
                ...
                powerDisplay.textContent = stats.power
                cookie.setItem(
                    '[gunn-web-app] scheduleapp.clickPower',
                    stats.power
                )
                priceDisplay.textContent = (stats.power + 1) * 25
            }
...
export function initLists () {
    ...
    if (loadLists) {
        ...
        toEach('.lists-enabled button', t =>
            t.addEventListener(
                ...
                e => {
                    cookie.setItem(
                        '[gunn-web-app] scheduleapp.loadLists',
                        'yes'
                    )
                    window.location.reload()
                },
    ...
    const { showItem: showClub, getCurrent: getCurrentClub } = initList(
        'club',
        {
