    ...
    return Promise.all(
        ...
        results
            ...
            .forEach(events =>
                Object.assign(
                    alternateSchedules,
                    toAlternateSchedules(events.items)
                )
            )
...
function initPSA () {
    fetch('./psa/psas.json')
        ...
        .then(psaData => {
            ...
            function displayPsa (id) {
                ...
                return Promise.resolve(
                    ...
                    if (currentPsa === id) {
                        const [year, month, date] = psaData[id]
                            .split('-')
                            .map(Number)
                        ...
                        psaContent.innerHTML =
                            html + `<p class="psa-date">${dateStr}</p>`
                        newBadge.style.display =
                            currentPsa > lastRead ? 'inline' : null
        ...
        .catch(err => {
            logError(err)
            document.getElementById('psa').textContent =
                localize('psa-error') + err
        })
}
...
function initGradeCalc () {
    ...
    function setOutput () {
        ...
        const result =
            Math.round(((minimum - current * (1 - worth)) / worth) * 10000) /
            100
        if (result <= 0) {
...
function initControlCentre () {
    ...
    document.getElementById('kill-sw').addEventListener('click', e => {
        navigator.serviceWorker.getRegistrations().then(regis =>
            regis.map(regis => {
                if (regis.scope.includes('gunn-web-app'))
...
function initSaveCodeManager () {
    ...
    function getExportCode () {
        ...
        for (let i = cookie.length; i--; ) {
            ...
            if (
                key.slice(0, UGWA_COOKIE_PREFIX.length) === UGWA_COOKIE_PREFIX
            ) {
                toExport[key.slice(UGWA_COOKIE_PREFIX.length)] = cookie.getItem(
                    key
                )
            } else if (key === EXCEPT) {
        ...
        fetch('./chats.txt?v=' + currentTime())
                    ...
                    while ((match = illegalChars.exec(input.value))) {
                        preview.appendChild(
                            document.createTextNode(
                                input.value.slice(i, match.index)
                            )
                        )
                        ...
                        strike.appendChild(
                            document.createTextNode(
                                input.value.slice(match.index, i)
                            )
                        )
                        preview.appendChild(strike)
                    }
                    preview.appendChild(
                        document.createTextNode(input.value.slice(i))
                    )
            ...
            .then(async () => {
                ...
                function getMessages () {
                    ...
                    if (
                        cookie.getItem('[gunn-web-app] section') !== 'utilities'
                    ) {
                        nextMessageGetTimeoutID = setTimeout(
                            getMessages,
                            FETCH_DELAY
                        )
                        return
                    ...
                    fetch(jsonStore)
                        ...
                        .then(({ result: messages }) => {
                            const isAtBottom =
                                output.scrollHeight - output.scrollTop ===
                                output.clientHeight
...
function localizePage () {
    ...
    function applyL10nToNodeFromStr (node, l10nStr) {
        ...
        for (const child of [...node.childNodes]) {
            if (child.nodeType === Node.ELEMENT_NODE) {
                if (child.dataset.l10nArg) {
                    ...
                    console.warn(
                        child,
                        'does not have a data-l10n-arg set for',
                        l10nStr
                    )
                }
            }
        ...
        if (extraArgs.length) {
            console.warn(
                'Extra l10n arguments found for',
                l10nStr,
                ':',
                extraArgs
            )
                ...
                regis.onupdatefound = () => {
                    ...
                    installingWorker.onstatechange = () => {
                        if (
                            ...
                            console.log(
                                'New update! Redirecting you away and back'
                            )
