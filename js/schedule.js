...
import { toLocalTime } from './timezone.js'
import {
    ajax,
    ALT_KEY,
    closeDialog,
...
function makeHMTM (hour, minute = 0) {
    const [lhour, lminute] = toLocalTime(hour, minute)
    return { hour: lhour, minute: lminute, totalminutes: hour * 60 + minute }
}
                ...
                if (currentDate) {
                    if (line[0] === '*') {
                        if (schedules[currentDate].description) {
                            schedules[currentDate].description +=
                                ' ' + line.slice(1).trim()
                        } else {
                            schedules[currentDate].description = line
                                .slice(1)
                                .trim()
                        }
                    } else if (line) {
                        ...
                        if (match) {
                            ...
                            const period =
                                periodLowercase === 'self'
                                    ? 'SELF'
                                    : periodLowercase[0].toUpperCase() +
                                      periodLowercase.slice(1)
...
export function initSchedule (manualAltSchedulesProm) {
    ...
    if (cookie.getItem('[gunn-web-app] scheduleapp.options')) {
        ...
        if (options[0] !== VERSION) {
            if (options[0] <= 1) {
                options.push([
                    periodstyles.SELF.label,
                    periodstyles.SELF.colour
                ])
            }
            if (options[0] <= 2) {
    ...
    toEach('input[name=format]', t =>
        t.addEventListener(
            ...
            e => {
                formatOptions[2] =
                    e.target.value === 'full' ? 'full' : 'compact'
                cookie.setItem(
                    '[gunn-web-app] scheduleapp.formatOptions',
                    formatOptions.join('.')
                )
    ...
    togglePdAsgn.parentNode.addEventListener('click', e => {
        ...
        formatOptions[6] = togglePdAsgn.classList.contains('checked')
            ? 'yes'
            : 'no'
        cookie.setItem(
            '[gunn-web-app] scheduleapp.formatOptions',
    ...
    if (formatOptions[10] === 'yes-h-period2')
        pd8Switch.classList.add('checked')
            ...
            .map(pd => {
                ...
                ;(periodstyles[pd].update = () => {
                    ...
                    if (periodstyles[pd].colour[0] === '#') {
                        ...
                        span.style.color = getFontColour(
    ...
    function asgnEditor ({ text, category, importance, dueObj, period }) {
        ...
        onDeleteClick = () => {
            ...
            cookie.setItem(
                '[gunn-web-app] assignments',
                asgnThing.getSaveable()
            )
        }
        onSaveClick = () => {
            currentCancelFn()
                ...
                for (let i = 0; i < items.length; i++) {
                    ...
                    if (items[i].start) {
                        ...
                        if (formatOptions[1] === '0')
                            ...
                            timerange = `${((start.getHours() - 1) % 12) +
                                1}:${('0' + start.getMinutes()).slice(-2)}${
    ...
    function identifyPeriod (name) {
        ...
        if (~name.indexOf('self')) return 'SELF'
            ...
            return 'Flex'
        else if (~name.indexOf('brunch') || ~name.indexOf('break'))
            return 'Brunch'
        else if (~name.indexOf('unch') || ~name.indexOf('turkey'))
            return 'Lunch'
    ...
    let alternates
    if (cookie.getItem(ALT_KEY))
        alternates = JSON.parse(cookie.getItem(ALT_KEY))
    else alternates = {}
    ...
    const scheduleapp = scheduleApp({
        ...
        customSchedule (date, y, m, d, wd) {
            ...
            const {
                periods,
                description = localize('default-alt-msg')
            } = schedule
            return {
                periods:
    ...
    scheduleAppWrapper.addEventListener('pointermove', e => {
        if (swiping && swiping.pointerId === e.pointerId) {
            ...
            if (swiping.swiping) {
                ...
                scheduleapp.container.style.opacity =
                    1 - Math.abs(swipeX) / width
                ...
                if (imageInput.input.value) {
                    cacheBackground(imageInput.input.value, id)
                        .then(() => {
                            ...
                            makeWeekHappen()
                            options[letras.indexOf(id)][1] =
                                imageInput.input.value
                            if (periodstyles[id].update)
                                periodstyles[id].update()
                            cookie.setItem(
                                '[gunn-web-app] scheduleapp.options',
                                JSON.stringify(options)
                            )
    ...
    // if (formatOptions[10] === 'yes-h-period2') {
    addCustomiser(
        localizeWith('periodx', 'other', { X: '8' }),
        'H',
        options[12]
    )
    // }
    ...
    openInFrame.parentNode.addEventListener('click', e => {
        ...
        formatOptions[14] = openInFrame.classList.contains('checked')
            ? ''
            : 'yes'
        scheduleapp.options.openLinkInIframe =
            formatOptions[14] !== 'yes' && setIframe
